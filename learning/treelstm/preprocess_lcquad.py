...
from copynews.qald import QaldParser

path = os.getcwd()
sys.path.insert(0, path)
from common.utility.utility import find_mentions
from copynews.lc_quad_linked import LC_Qaud_LinkedParser
...
def generalize_question(a, b, parser=None):
    ...
    b, supported, uris = parser.parse_sparql(b)
...
def split(data, parser=None):
        ...
        if "query" in raw_row:
            if isinstance(raw_row["query"], dict):
                if "sparql" in raw_row["query"]:
                    ...
                    b_list.append(b[0].encode('ascii', 'ignore').decode('ascii') + '\n')
                    sim_list.append("2" + '\n')
            else:
                query = raw_row["query"]
    ...
    for item in tqdm(dataset):
        ...
        for query in item["generated_queries"]:
            ...
            sim_list.append("2" + '\n')
    with open(os.path.join("./output/", 'a.txt'), 'w') as afile, \
            open(os.path.join("./output/", 'b.txt'), 'w') as bfile, \
            open(os.path.join("./output/", 'id.txt'), 'w') as idfile, \
            open(os.path.join("./output/", 'sim.txt'), 'w') as simfile:
...
if __name__ == '__main__':
    ...
    path = os.path.join(lc_quad_dir, 'lcquad_gold.json')
    ...
    parser = LC_Qaud_LinkedParser()
    #parser = QaldParser()
    #train_filepath = 'D:\\downloads\\QA\\data\\QALD\\qald-9-train.json'
